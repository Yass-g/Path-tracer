...
vector < Object * > Scene = {
  ...
  new Sphere(26.5, Vect(60, 26.5, 22), Vect(), Vect(1, 1, 1) * .596, SPEC),
  new Sphere(13, Vect(22, 13, 62), Vect(), Vect(.96, .96, .96) * .96, REFR),
...
Vect radiance(const Ray & r_, int depth_) { //solving the rendering equation (Monte Carlo+Russian roulette)
  ...
  while (1) {
    ...
    Vect x = r.o + r.d * t, n = obj -> normal(x), nl = n.dot(r.d) < 0 ? n : n * -1, f = obj -> c; //x : intersection point, n : sphere normal,
                                                                                                 //n1 : corrected normal, f : object color
    Ray ref_Ray(x, r.d - n * 2 * n.dot(r.d));
    double p = f.x > f.y && f.x > f.z ? f.x : f.y > f.z ? f.y : f.z;                        // max refl
    cl = cl + cf.mult(obj -> e);
    if (++depth > 5)
      if (erand48(1) < p) f = f * (1 / p);
      else return cl;                                                                     //Russian roulette after (at least) a depth of 5
    cf = cf.mult(f);
    // DIFFUSE reflection
    if (obj -> refl == DIFF) {
      double r1 = 2 * M_PI * erand48(1), r2 = erand48(1), r2s = sqrt(r2);
...
int main(int argc, char * argv[]) {
  ...
  // generating image 
  //multi-threaded rendering with OpenMP library
